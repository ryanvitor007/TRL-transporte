...
export interface JornadaMonitoramento {
  ...
  vehiclePlate: string;
  vehicleModel: string;
  status: 'pending_approval' | 'active' | 'resting' | 'meal' | 'finished' | 'cancelled';
  startTime: string;
  startLocation: string;
...
function adapterJornadaMonitoramento(data: any): JornadaMonitoramento {
  ...
  return {
    ...
    vehicleModel: data.vehicle?.modelo ? `${data.vehicle.marca} ${data.vehicle.modelo}` : data.vehicleModel || 'N/A',
    status: data.status,
...
export async function buscarJornadasMonitoramentoAPI(): Promise<JornadaMonitoramento[]> {
  try {
...
export async function buscarHistoricoJornadasDiaAPI(): Promise<JornadaMonitoramento[]> {
  try {
    const today = new Date().toISOString().split('T')[0];
    const response = await fetch(`${API_BASE_URL}/journeys/history?date=${today}`, {
      method: 'GET',
      headers: buildHeaders(),
      cache: 'no-store',
    });
...
export async function autorizarJornadaComRiscoAPI(journeyId: number, adminNotes?: string) {
  const response = await fetch(`${API_BASE_URL}/journeys/${journeyId}/authorize`, {
    method: 'PATCH',
    headers: buildHeaders(),
    body: JSON.stringify({ 
      status: 'active',
...
export async function buscarStatusJornadaAPI(journeyId: number): Promise<string | null> {
  try {
    const response = await fetch(`${API_BASE_URL}/journeys/${journeyId}/status`, {
      method: 'GET',
      headers: buildHeaders(),
      cache: 'no-store',
    });
