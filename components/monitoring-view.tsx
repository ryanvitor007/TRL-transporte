...
export function MonitoringView() {
  ...
  // Modais
  const [selectedJourney, setSelectedJourney] = useState<JornadaMonitoramento | null>(null);
  const [isDetailsOpen, setIsDetailsOpen] = useState(false);
  const [emergencyJourney, setEmergencyJourney] = useState<JornadaMonitoramento | null>(null);
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-4">
        {kanbanColumns.map((column) => {
          const columnJourneys = grouped[column.id as keyof typeof grouped] || [];
          const ColumnIcon = column.icon;

          return (
            <Card key={column.id} className={cn("min-h-[400px]", column.borderColor)}>
              <CardHeader className={cn("pb-3", column.bgColor)}>
                <CardTitle className="flex items-center justify-between text-sm">
                  ...
                  <Badge
                    ...
                    className={cn(
                      "font-mono",
                      column.id === "pending" && columnJourneys.length > 0 && "bg-red-500 text-white"
                    )}
                  >
              ...
              <CardContent className="p-3">
                {isLoading ? (
                  ...
                  <div className="flex h-32 flex-col items-center justify-center text-center">
                    ...
                    <p className="text-xs text-muted-foreground">{column.emptyText}</p>
                  </div>
                ) : (
                    ...
                    <div className="space-y-3">
                      {columnJourneys.map((journey) => (
                        <JourneyKanbanCard
                          ...
                          variant={column.id === "pending" ? "critical" : "default"}
                        />
                      ))}
                    </div>
                  </ScrollArea>
